...
(function() {
  ...
  Card = (function() {
    ...
    Card.prototype.toString = function() {
      if (this.isWarrior) {
        return "" + this.type + " " + this.energy + " (" + this.attack + "/" + this.defense + ")";
      } else {
        return "" + this.type;
      }
    };
  ...
  Player = (function() {
    ...
    function Player(data, socket) {
      ...
      this.opponent = null;
      this.hand = data.hand, this.turn = data.turn, this.deck = data.deck, this.name = data.name;
    }
    ...
    Player.prototype.performAction = function(action, result) {
      var card, from, removed, to, _i, _len, _ref;
      switch (action.type) {
        case 'draw':
          this.deck.pop();
      ...
      this.updateGUI(action);
      return true;
    };

    Player.prototype.collectDeath = function() {
      var i, _i, _ref, _results;
      _results = [];
      for (i = _i = 0, _ref = GameRules.fieldSize - 1; 0 <= _ref ? _i <= _ref : _i >= _ref; i = 0 <= _ref ? ++_i : --_i) {
        if (this.field[i] !== null && this.field[i].energy <= 0) {
          this.field[i] = null;
        }
        if (this.opponent.field[i] !== null && this.opponent.field[i].energy <= 0) {
          _results.push(this.opponent.field[i] = null);
        } else {
